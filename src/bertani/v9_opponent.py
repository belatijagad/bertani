...
class V9SelfPlayEnv:
    """Compose one learner seat and one frozen V9 seat in each vector slot."""

    def __init__(
        self,
        environment: VecEnv,
        opponent: V9OpponentPolicy,
    ) -> None:
        self.environment = environment
        self.opponent = opponent
        self.games = np.arange(environment.num_envs, dtype=np.int64)
        # Alternating seats removes first/second-player bias in every batch.
    ...
    def step(
        self,
        learner_unit_actions: NDArray[np.int64],
        learner_market_actions: NDArray[np.int64] | None = None,
        learner_market_lengths: NDArray[np.int64] | None = None,
    ) -> Batch:
        ...
        if learner_market_lengths is not None and learner_market_actions is None:
            raise ValueError(
                "learner_market_lengths requires learner_market_actions"
            )
